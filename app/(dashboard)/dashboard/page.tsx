...
function injectDashStyles() {
  ...
  s.textContent = `
    ...
    .ops-stat-label { font-size:13px; letter-spacing:0.1em; color:#78899a; margin-top:3px; text-transform:uppercase; }

    .ops-feed-row {
      padding:10px 16px; border-bottom:1px solid #181d22;
    ...
    .health-row {
      ...
      padding:10px 16px; border-bottom:1px solid #181d22; font-size:12px;
    }
            ...
            <div key={cam.id} style={{
              ...
              >
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ fontSize: 12, color: C.txt3, marginTop: 1, letterSpacing: '0.06em' }}>{cam.id.slice(-8).toUpperCase()}</div>
                </div>
                ...
                <span style={{ fontSize: 12, color: C.txt3, marginLeft: 6, flexShrink: 0 }}>{isExpanded ? '▲' : '▼'}</span>
              </div>
  ...
  return (
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '5px 12px', borderBottom: `1px solid ${C.border}`, flexShrink: 0 }}>
        <span style={{ fontSize: 12, color: C.txt3, letterSpacing: '0.12em' }}>TRACKING FEED — CLICK PLATE FOR JOURNEY</span>
...
export default function OpsDashboard() {
  ...
  const faceEvents: any[] = (() => {
    // Merge live SSE events with polled data, deduplicate by id
      ...
      <div style={{ height: 52, background: C.bg1, borderBottom: `1px solid ${C.border}`, display: 'flex', alignItems: 'stretch', flexShrink: 0, zIndex: 100 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, paddingLeft: 14, paddingRight: 18, borderRight: `1px solid ${C.border}`, flexShrink: 0 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
            ...
            <div style={{ fontSize: 12, color: C.txt3, letterSpacing: '0.1em', textTransform: 'uppercase' }}>Multiple Identity Tracking System</div>
          </div>
        </div>
...
// ─── Person Face Journey View ─────────────────────────────────────────────────

function PersonFaceJourneyView({ personId, persons, cameras, C }: {
  personId: string
  persons: Person[]
  cameras: Camera[]
  C: Record<string, string>
